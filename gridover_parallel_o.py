...
def main(trial):
    ...
    oarray = np.arange(0.0, 0.11,0.01)
        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
               ...
               """
                ...
                plt.savefig('frate_%s_%s_%s.png'%(EP,TR, OV))
                plt.close(fig)
    ...
    pickle.dump(csfrac,open('csfraco4_{0}.pkl'.format(TR),'wb'))
    pickle.dump(isfrac,open('isfraco4_{0}.pkl'.format(TR),'wb'))
    pickle.dump(csrate,open('csrateo4_{0}.pkl'.format(TR),'wb'))
    pickle.dump(israte,open('israteo4_{0}.pkl'.format(TR),'wb'))
