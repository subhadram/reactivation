...
def main(trial):
    ...
    aarray = np.arange(1.2e-9,1.21e-9,5e-11)
    epsilon = np.arange(.0e-9, .03e-9, 5e-11)
    ...
    mratecs = np.zeros((len(aarray),len(epsilon), trials))
    mrateis = np.zeros((len(aarray),len(epsilon), trials))
    ...
    for k in range(len(aarray)):   
        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
                ...
                with open('strcf.pkl','rb') as f: 
                    strc = np.squeeze(pickle.load(f,encoding = 'latin1'))
                #pickle.dump(strc,open('strcf.pkl','wb'))
                J = J * strc
                ...
                with open('Vin.pkl','rb') as f: 
                    V[:,0] = np.squeeze(pickle.load(f,encoding = 'latin1'))
                #pickle.dump(V[:,0],open('Vin.pkl','wb'))
                #w[:,0] = np.squeeze(w00) #
                with open('win.pkl','rb') as f: 
                    w[:,0] = np.squeeze(pickle.load(f,encoding = 'latin1'))
                #w[:,0] = dw*np.random.rand(N)
                #pickle.dump(w[:,0],open('win.pkl','wb'))
               ...
               """
                ...
                with open('ranr.pkl','rb') as f: 
                    ran_r = np.squeeze(pickle.load(f,encoding = 'latin1'))
                #pickle.dump(ran_r,open('ranr.pkl','wb'))
                ...
                #noise_EE = gnoise_E * (poisson.rvs(rnoise_EE * dt, size=(NE, Nt)) +np.outer(np.ones(NE), poisson.rvs(rnoise_EE_corr * dt, size=Nt)))
                #noise_EE = noise_EE * (np.random.rand(NE, Nt) < np.outer(np.ones(NE), r_factor))
    
                #noise_IE = gnoise_I * (poisson.rvs(rnoise_IE * dt, size=(NE, Nt)) +np.outer(np.ones(NE), poisson.rvs(rnoise_IE_corr * dt, size=Nt)))
                #noise_EI = gnoise_E * (poisson.rvs(rnoise_EI * dt, size=(NI, Nt)) +np.outer(np.ones(NI), poisson.rvs(rnoise_EI_corr * dt, size=Nt)))
                #noise_II = gnoise_I * (poisson.rvs(rnoise_II * dt, size=(NI, Nt)) +np.outer(np.ones(NI), poisson.rvs(rnoise_II_corr * dt, size=Nt)))
                
                #pickle.dump(noise_EE,open('noiseee.pkl','wb'))
                #pickle.dump(noise_IE,open('noiseie.pkl','wb'))
                #pickle.dump(noise_EI,open('noiseei.pkl','wb'))
                #pickle.dump(noise_II, open('noiseii.pkl','wb'))
                
                with open('noiseee.pkl','rb') as f: 
                    noise_EE = np.squeeze(pickle.load(f,encoding = 'latin1'))
                with open('noiseie.pkl','rb') as f: 
                    noise_IE = np.squeeze(pickle.load(f,encoding = 'latin1'))
                with open('noiseei.pkl','rb') as f: 
                    noise_EI = np.squeeze(pickle.load(f,encoding = 'latin1'))
                with open('noiseIi.pkl','rb') as f: 
                    noise_II = np.squeeze(pickle.load(f,encoding = 'latin1'))
                ...
                plt.savefig('frate_%s_%s.png'%(EP,TR))
                plt.show()
                # plt.close(fig)
                # fig = plt.figure(2)
                # plt.hist(rateov)
                ...
                mratecs[k,eps,j] = np.mean(rateg1)
                mrateis[k,eps,j] = np.mean(rateg2)
                print(k,eps,j)
    ...
    #pickle.dump( rateg1, open( "cstrace1_%s.pkl","wb")%(TR))
    #pickle.dump( rateg2, open( "istrace1_%s.pkl","wb")%(TR))
    #pickle.dump( rateov, open( "intrace1_%s.pkl","wb")%(TR))
    pickle.dump(rateg1,open('cstrace1_{0}.pkl'.format(TR),'wb'))
    pickle.dump(rateg2,open('istrace1_{0}.pkl'.format(TR),'wb'))
    pickle.dump(rateov,open('intrace1_{0}.pkl'.format(TR),'wb'))
    #pickle.dump(csrate,open('csrateinh_{0}.pkl'.format(TR),'wb'))
    #pickle.dump(israte,open('israteinh_{0}.pkl'.format(TR),'wb'))
