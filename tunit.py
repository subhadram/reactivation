...
def main(trial):
    ...
    xi = [0.0101,0.01]
    xi = np.array(xi)
    xlast1 = np.zeros((len(ga),len(epsilon),len(aa)))
    xlast2 = np.zeros((len(ga),len(epsilon),len(aa)))
    
    
    fractimeis = np.zeros((len(ga),len(epsilon),len(aa),len(mus)))
    fractimecs = np.zeros((len(ga),len(epsilon),len(aa),len(mus)))
    uptimecs = np.zeros((len(ga),len(epsilon),len(aa)))
    uptimeis = np.zeros((len(ga),len(epsilon),len(aa)))
    ...
    for k in range(0,len(ga)):
        ...
        for j in range(0, len(epsilon)):
          ...
          for l in range(len(aa)):
            ...
            fractimetemp = np.zeros(len(mus))
            fractimetempcs = np.zeros(len(mus))
            fractimetempis = np.zeros(len(mus))
           
            for h in range(0,len(mus)):
                mu = mus[h]
                MUM = str(mu)
                x[k,j,l,h,0,0] = xi[0]
                x[k,j,l,h,1,0] = xi[1]
                ...
                fractimecs[k,j,l,h] = np.mean(frcs)
                fractimeis[k,j,l,h] = np.mean(fris)
                #print(frcs)
                ...
                plt.ylabel(r"$x$")
                plt.ylim(-0.1,1.1)
                plt.title(r"g =" + GG + r" $\epsilon$ = "+ EP + " a =" + AA + r" $\sigma_x$ = " + NAMP + " $\mu$ = " + MUM)
                plt.savefig('xx_g_%s_ep_%s_a_%s_ic_%s_na_%s_tr_%s.png'%(GG, EP,AA,LL,NAMP,MUM))
                plt.close(fig)       
    ...
    #print(fractimecs)
    #print(fractimeis)
    #pickle.dump(csfrac,open('csfraca8_{0}.pkl'.format(TR),'wb'))
    pickle.dump( time_to_first_transition, open( "timetotranc_{0}.pkl".format(TR),"wb"))
    
    
    pickle.dump( trans_to_csstate, open( "transprobc_{0}.pkl".format(TR),"wb"))
    
    pickle.dump( start_state_ll, open( "startstatec_{0}.pkl".format(TR),"wb"))
    pickle.dump( fractimecs, open( "fracttimecsc_{0}.pkl".format(TR),"wb"))
    pickle.dump( fractimeis, open( "fracttimeisc_{0}.pkl".format(TR),"wb"))
