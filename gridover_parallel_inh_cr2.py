...
def main(trial):
    ...
    epsilon = np.arange(-0.1e-9, .11e-9, 10e-11)
               ...
               """
                ...
                for ie2,ei2 in enumerate(event_indices2):
                    ...
                    if (ei2 > 10000) and all( xr <10 for xr in rateg1[ei2-10000:ei2]):
                        ...
                        #plt.plot(tvector, rateov)
                        plt.title(r" $\epsilon$ = "+ EP)
                        plt.savefig('istrigger2_%s_%s.png'%(EP,ie))
                        #plt.show()
                        plt.close(fig)
                
                tttime = np.arange(-int(10000*dt),0,dt)
                plt.plot(tttime,np.mean(triggered_average1,0), label = "CS event")
                plt.plot(tttime,np.mean(triggered_average2,0), label = "IS event")
                plt.ylabel("inhibitory firing rate ")
                plt.xlabel("time before event")
                plt.title(r" $\epsilon$ = "+ EP)
                plt.savefig('sta2_%s_%s.png'%(EP,ie))
