...
def main(trial):
    ...
    epsilon = np.arange(0.0e-9, .01e-9, 1e-11)
    ...
    for k in range(len(aarray)):   
        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
                ...
                fac = np.zeros((N,len(tvector)))
                faci = np.zeros((N,len(tvector)))
               ...
               """
                ...
                noise_EE[group2ei:inhind,:] = noise_EE[group2ei:inhind,:] * (np.random.rand(int(0.05*N), Nt) < np.outer(np.ones(int(0.05*N)), r_factor3))
                ...
                for tt in range(len(tvector)-1):
                    ...
                    cellsfirede = cellsfired[cellsfired< inhind]
                    cellsfiredi = cellsfired[cellsfired>= inhind]
                    #print(cellsfirede)
                    ...
                    fac[cellsfired,tt] = fac[cellsfired,tt] + ffac*(Fmax - fac[cellsfired,tt])
                    ...
                    fac[:,tt+1] = 1.0 - (1.0-fac[:,tt])*np.exp(-dt/tauf)
                    #faci[:,tt+1] = fac[:,tt]+1
                plt.plot(tvector,s[0,:])
    ...
    """
    pickle.dump(mratecsd,open('u3dratecs_{0}.pkl'.format(TR),'wb'))
    pickle.dump(mrateisd,open('u3drateis_{0}.pkl'.format(TR),'wb'))
    pickle.dump(mratecsb,open('u3bratecs_{0}.pkl'.format(TR),'wb'))
    pickle.dump(mrateisb,open('u3brateis_{0}.pkl'.format(TR),'wb'))
    pickle.dump(mratecsa,open('u3aratecs_{0}.pkl'.format(TR),'wb'))
    pickle.dump(mrateisa,open('u3arateis_{0}.pkl'.format(TR),'wb'))
    pickle.dump(csfrac,open('u3mcsfrac_{0}.pkl'.format(TR),'wb'))
    pickle.dump(isfrac,open('u3misfrac_{0}.pkl'.format(TR),'wb'))
    pickle.dump(lencs,open('u3numcs_{0}.pkl'.format(TR),'wb'))
    pickle.dump(lenis,open('u3numis_{0}.pkl'.format(TR),'wb'))
    pickle.dump(csrate,open('u3mcsrate{0}.pkl'.format(TR),'wb'))
    pickle.dump(israte,open('u3misrate_{0}.pkl'.format(TR),'wb'))
    pickle.dump(mratecstotal,open('u3mratecst_{0}.pkl'.format(TR),'wb'))
    pickle.dump(mrateistotal,open('u3mrateist_{0}.pkl'.format(TR),'wb'))
