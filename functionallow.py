...
for tr in range(1,101):
    TR = str(tr)
    with open('csfraca9_{0}.pkl'.format(TR),'rb') as f: 
        csfrac[:,:,tr-1] = np.squeeze(pickle.load(f,encoding = 'latin1'))
    with open('isfraca9_{0}.pkl'.format(TR),'rb') as f:
...
timea = np.arange(0,100000,0.1)
def expo(t,k,a0,ab):
    y = 12e-10*(0.1*np.exp(-k*t) + 0.3*np.exp(-k*t/40.0) )+ ab 
    return y

expon = expo(timea, 0.0005, 0.01, 1.0e-9)
expon2 = expo(timea-3000,0.0005, 0.1, 1.0e-9)
...
timea = np.arange(0,100000,0.1)
def expo(t,k,a0,ab):
    y = 25e-10*(0.3*np.exp(-k*t/0.5) + 0.1*np.exp(-k*t/18.0) )+ ab 
    return y
