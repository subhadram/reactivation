        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
               ...
               """
                ...
                r_factor1 = np.maximum(filt_rate + 0.5, 0)
                ...
                filt_rate = filt_rate - np.mean(filt_rate)
                filt_rate = filt_rate / np.sqrt(dtf)
                r_factor2 = np.maximum(filt_rate + 0.5, 0)
                #plt.plot(r_factor)
                #plt.show()
                #r_factor = 0.8
                ...
                filt_rate = filt_rate - np.mean(filt_rate)
                filt_rate = filt_rate / np.sqrt(dtf)
                r_factor3 = np.maximum(filt_rate + 0.5, 0)
                ...
                noise_EE[0:group1ei,:] = noise_EE[0:group1ei,:] * (np.random.rand(Ng1, Nt) < np.outer(np.ones(Ng1), r_factor1))
                noise_EE[group2si:group2ei,:] = noise_EE[group2si:group2ei,:] * (np.random.rand(Ng2, Nt) < np.outer(np.ones(Ng2), r_factor2))
                noise_EE[group2ei:inhind,:] = noise_EE[group2ei:inhind,:] * (np.random.rand(int(0.05*N), Nt) < np.outer(np.ones(int(0.05*N)), r_factor3)) #0.05 is the portion of excitatory neurons not in either ensemble.
                ...
                allrates[1,:] = rateg2
                allrates[2,:] = rateov
                pickle.dump(allrates,open('allratesuc_{0}.pkl'.format(TR),'wb'))
