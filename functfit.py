...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 29 15:56:23 2024
...
dt = 1e-4
warray = np.arange(1.0e-9,1.51e-9,.025e-9)
epsilon = np.arange(0.0e-9, 0.21e-9, 0.1e-10)
#epsilon = np.arange(0.0e-9,1.0e-9, 5e-11)
#w0array = np.arange(0e-9,2.1e-9, 0.1e-9)
#warray = np.arange(1.5e-9, 2.55e-9,5e-11)
...
for tr in range(1,101):
    TR = str(tr)
    with open('csfracag_{0}.pkl'.format(TR),'rb') as f: 
        csfrac[:,:,tr-1] = np.squeeze(pickle.load(f,encoding = 'latin1'))
    with open('isfracag_{0}.pkl'.format(TR),'rb') as f:
...
timea = np.arange(0,30000,0.1)
def expo(t,k,a0,ab):
    y = 12e-10*(0.3*np.exp(-k*t/20) + 0.1*np.exp(-k*t) )+ ab 
    return y

expon = expo(timea, 0.0005, 0.01, 1.0e-9)
expon2 = expo(timea-2800,0.0005, 0.01, 1.0e-9 )
...
timea = np.arange(0,30000,0.1)
def expo(t,k,a0,ab):
    y = 25e-10*(0.1*np.exp(-k*t) + 0.3*np.exp(-k*t/12.0) )+ ab 
    return y
