...
J = sio.loadmat('J.mat')
J = J['J']
spike401 = sio.loadmat('spikes401.mat')
spike401 = spike401['spikes401']
print(np.mean(J))
plt.imshow(J)
plt.show()
...
aa = np.arange(1.2e-9,1.4e-9,5e-10)
epsilon = np.arange(-0.5e-9, 0.55e-9, 1e-10)
overlaps = np.arange(0.0, 0.06, 0.05)
firingrates = np.zeros((len(aa),len(overlaps),trials,3,len(tvector)))
...
for k in range(len(aa)):   
    ...
    for eps in range(len(epsilon)):
        ...
        for j in range(trials):
            ...
            noise_EE = gnoise_E*np.random.poisson(rnoise_EE*dt, [NE, Nt])
            noise_EE = gnoise_E * (poisson.rvs(rnoise_EE * dt, size=(NE, len(tvector))) + np.outer(np.ones(NE), poisson.rvs(rnoise_EE_corr * dt, size=len(tvector))))
            onep = np.ones((NE,1))*r_factor
            print(onep.shape)
            noise_EE = noise_EE*np.array(np.where(np.random.rand(NE, Nt) < onep,1,0))
            noise_IE = gnoise_I*np.random.poisson(rnoise_EE*dt, [NE, Nt])
            noise_EI = gnoise_E*np.random.poisson(rnoise_EE*dt, [NI, Nt])
            noise_II = gnoise_I*np.random.poisson(rnoise_EE*dt, [NI, Nt])
            ...
            for ti in range(1,len(tvector)):
                gnoise_EE[:,ti-1] = gnoise_EE[:,ti-1] + noise_EE[:,ti-1]
                gnoise_IE[:,ti-1] = gnoise_IE[:,ti-1] + noise_IE[:,ti-1]
                gnoise_EI[:,ti-1] = gnoise_EI[:,ti-1] + noise_EI[:,ti-1]
                gnoise_II[:,ti-1] = gnoise_II[:,ti-1] + noise_II[:,ti-1]
                
                gnoise_EE[:,ti] = gnoise_EE[:,ti-1]*np.exp(-dt/tau_noiseE)
                gnoise_IE[:,ti] = gnoise_IE[:,ti-1]*np.exp(-dt/tau_noiseI)
                gnoise_EI[:,ti] = gnoise_EI[:,ti-1]*np.exp(-dt/tau_noiseE)
                gnoise_II[:,ti] = gnoise_II[:,ti-1]*np.exp(-dt/tau_noiseI)
                
            plt.plot(gnoise_EE[1,:])
            plt.show()
            ...
            noise_EE = noise_EE*(np.random.rand(NE, len(tvector)) < np.outer(np.ones(NE), r_factor))
            ...
            print(np.mean(gnoise_EE))
            """
               
            gnoise_EE = sio.loadmat('gnoise_EE.mat')
            
            gnoise_EI = sio.loadmat('gnoise_EI.mat')
            gnoise_IE = sio.loadmat('gnoise_IE.mat')
            gnoise_II = sio.loadmat('gnoise_II.mat')
            gnoise_EE = gnoise_EE['gnoise_EE']
            gnoise_EI = gnoise_EI['gnoise_EI']
            gnoise_IE = gnoise_IE['gnoise_IE']
            gnoise_II = gnoise_II['gnoise_II']
           
            gnoise_EE = np.array(gnoise_EE)
            gnoise_EI = np.array(gnoise_EI)
            gnoise_II = np.array(gnoise_II)
            gnoise_IE = np.array(gnoise_IE)
            print(np.mean(gnoise_EE))
            J = np.array(J)
            """
            ...
            plt.plot(tvector, np.squeeze(spike401),'o',alpha = 0.5)
            plt.plot(tvector, spikes[400,:],'o',alpha = 0.5)
            plt.show()
            print(np.mean(np.sum(spikes[:,:],1)), np.sum(spike401))
            fig = plt.figure(1)
...
#pickle.dump( firingrates, open( "firingratesnn.pkl","wb"))

plt.plot(epsilon, np.mean(csfrac,1))
plt.plot(epsilon, np.mean(isfrac,1))
plt.show()
