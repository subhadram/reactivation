...
for i in range(len(epsilon)):
    for j in range(trials):
        ...
        I = np.sqrt(dt)*np.random.normal(0,150e-8,[N,len(tvector)])
        #I[200:400,:] = 0.0
        #I[:,10000:] = 0.0
...
plt.plot(epsilon,np.mean(fractc,1), label =" CS" )
plt.plot(epsilon, np.mean(fracti,1), label ="IS" )
plt.ylabel("fraction of time spent in state")
plt.xlabel(r"$epsilon$")
plt.legend()
plt.show()

plt.plot(epsilon,np.mean(spikescs,1), label = "CS")
plt.plot(epsilon,np.mean(spikesis,1), label ="IS" )
plt.ylabel("spikes")
plt.xlabel(r"$epsilon$")
plt.legend()
plt.show()

plt.plot(epsilon,np.mean(maxratecs,1), label = "CS")
plt.plot(epsilon,np.mean(maxrateis,1), label ="IS" )
plt.ylabel("maximum firing rate")
plt.xlabel(r"$epsilon$")
plt.legend()
plt.show()
