...
with open('firingratessigma.pkl','rb') as f:
...
aa = np.arange(2.5e-9,3.0e-9,5e-10)
epsilon = np.arange(15e-8, 150e-8, 15e-8)
#firingrates = np.zeros((len(aa),len(epsilon),trials,3,len(tvector)))
fracti = np.zeros((len(epsilon),trials))
fractc = np.zeros((len(epsilon),trials))
maxratecs = np.zeros((len(epsilon),trials))
maxrateis = np.zeros((len(epsilon),trials))
spikescs = np.zeros((len(epsilon),trials))
spikesis = np.zeros((len(epsilon),trials))
for k in range(len(aa)):   
    ...
    for i in range(len(epsilon)):
        ...
        for j in range(trials):
            ...
            fracti[i,j] = np.mean(fractinis)
            fractc[i,j] = np.mean(fractincs)
            spikescs[i,j] = np.sum(spikesg1[1000:])
            spikesis[i,j] =  np.sum(spikesg2[1000:])
...
plt.errorbar(epsilon,np.mean(fractc,1), yerr = np.std(fractc,1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5, label = "CS")
plt.errorbar(epsilon,np.mean(fracti,1),yerr = np.std(fracti,1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5, label = "IS")
plt.xlabel(r"$\sigma$")
...
plt.errorbar(epsilon,np.mean(spikescs,1), yerr = np.std(spikescs,1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5, label = "CS")
plt.errorbar(epsilon,np.mean(spikesis,1), yerr = np.std(spikesis,1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5,  label = "IS")
plt.xlabel(r"$\sigma$")
...
plt.errorbar(epsilon, np.mean(maxratecs,1), yerr = np.std(maxratecs,1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5, label = "CS")
plt.errorbar(epsilon, np.mean(maxrateis,1),yerr = np.std(maxrateis,1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5, label = "IS")
