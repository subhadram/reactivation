...
def main():
    ...
    epsilon = np.arange(-1.0e-9, 1.1e-9, 1e-10)
    ...
    for k in range(len(aa)):   
        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
                ...
                J[group2si:group2ei,group1si:group1si] = J21
                J[group1si:group1si,group2si:group2si] = J12
...
#plt.errorbar(epsilon, np.mean(csfrac[0,:,:],1), yerr =np.std(csfrac[0,:,:],1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5, label = "CS")
#plt.errorbar(epsilon, np.mean(isfrac[0,:,:],1), yerr =np.std(isfrac[0,:,:],1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5, label = "IS")
