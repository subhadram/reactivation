...
def main(trial):
    ...
    epsilon = np.arange(0.0e-9, 1.1e-9, 5e-11)
            ...
            for j in range(trials):
               ...
               """
                ...
                for ie,ei in enumerate(event_indices):
                    keir.append(np.mean(rateov[ei:ei+500]))
                ...
                event_indices = []
                #print(high1[indices[0][0]])
                if len(indices[0])>0 and high2[indices[0][0]] ==0:
                    event_indices = indices[0][::2]
                if len(indices[0])>0 and high2[indices[0][0]] ==1:
                    event_indices = indices[0][1::2]
                keir2 = []
                for ie,ei in enumerate(event_indices):
                    keir2.append(np.mean(rateov[ei:ei+500]))
                ...
                mratecs[k,eps,j] = np.mean(keir)
                mrateis[k,eps,j] = np.mean(keir2)
                print(k,eps,j)
    ...
    pickle.dump(mratecs,open('miratecs_{0}.pkl'.format(TR),'wb'))
    pickle.dump(mrateis,open('mirateis_{0}.pkl'.format(TR),'wb'))
