...
def main(trial):
    ...
    for k in range(0,len(mus)):
        ...
        for j in range(0, len(epsilon)):
          ...
          for l in range(len(aa)):
            ...
            a = aa[l]
            AA = str(a)
            J = [[a+epsi,e, 0, -b],[e,0,d,-b],[0,d,a,-b],[0.8*c, 0.2*c, 0.8*c,0]]
            J = np.array(J)
            ...
            for h in range(0,trials):
                ...
                for i in range(0, len(timea)-1):
                    #print(x[k,j,l,h,:,i].shape, J.shape)
                ...
                fig = plt.figure(1)
                plt.plot(timea, x[k,j,l,h,0,:], label = "IS")
                plt.plot(timea, x[k,j,l,h,2,:],label = "CS")
                ...
                plt.savefig('x4_g_%s_ep_%s_a_%s_ic_%s_na_%s_tr_%s.png'%(GG, EP,AA,LL,NAMP,TR))
                plt.close(fig)       
    ...
    #print(fractimecs)
    #print(fractimeis)
    pickle.dump( time_to_first_transition, open( "timetotranc4_{0}.pkl".format(TR),"wb"))
    
    
    pickle.dump( trans_to_csstate, open( "transprob4c_{0}.pkl".format(TR),"wb"))
    
    pickle.dump( start_state_ll, open( "startstatec4_{0}.pkl".format(TR),"wb"))
    pickle.dump( fractimecs, open( "fracttimecsc4_{0}.pkl".format(TR),"wb"))
    pickle.dump( fractimeis, open( "fracttimeisc4_{0}.pkl".format(TR),"wb"))
