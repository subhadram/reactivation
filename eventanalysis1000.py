...
for j in range(10):
    ...
    #print(len(listAnA), len(listAnB),len(listBnA), len(listBnB))
    
    
    print(ks_2samp(listBnA, listBnB).pvalue, ks_2samp(listAnA, listAnB).pvalue)
...
plt.hist(listBnA,alpha =0.5,label = r"$IS \to CS$")
...
plt.hist(listAnB,alpha =0.5,label = r"$CS \to IS$")
plt.hist(listAnA,alpha =0.5,label = r"$CS \to CS$")
plt.legend()
plt.xlabel("(time(s)")
plt.show()

cstois =np.cumsum(listAnB)
