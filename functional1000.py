...
dt = 1e-4
#warray = np.arange(1.0e-9,1.51e-9,2.5e-11)
#epsilon = np.arange(0.0e-9, .5e-9, 2.5e-11)
...
for tr in range(1,1001):
    TR = str(tr)
    with open('csfraca8_{0}.pkl'.format(TR),'rb') as f: 
        csfrac[:,:,tr-1] = np.squeeze(pickle.load(f,encoding = 'latin1'))
    with open('isfraca8_{0}.pkl'.format(TR),'rb') as f:
...
timea = np.arange(0,40000,0.1)
def expo(t,k,a0,ab):
    y = 25e-10*(0.1*np.exp(-k*t) + 0.3*np.exp(-k*t/20.0) )+ ab 
    return y
plt.rcParams.update({'font.size': 14})
expon = expo(timea, 0.0005, 0.01, 1.5e-9)
expon2 = expo(timea-3000,0.0005, 0.1, 1.5e-9)
plt.plot(timea, expon,color = "forestgreen", label ="CS")
plt.plot(timea, expon2, color = "gold", label = "IS" )
plt.xlabel("time (s)")
...
plt.imshow(alldatais,origin='lower',extent = (min(epsilon), max(epsilon), min(warray), max(warray)),aspect = 'auto')
clb = plt.colorbar()
...
timea = np.arange(0,40000,0.1)
def expo(t,k,a0,ab):
    y = 25e-10*(0.1*np.exp(-k*t) + 0.3*np.exp(-k*t/1.0) )+ ab 
    return y
#expon = expo(timea, 0.0005, 0.01, 1.5e-9)
def sigmoid(x, L ,x0, k, b):
    y = L / (1 + np.exp(-k*(x-x0))) + b
...
trajc = sigmoid(10**10*expon2,*popt)
