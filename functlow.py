...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 13 14:25:19 2024
...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct  5 14:17:11 2024
...
dt = 1e-4
warray = np.arange(1.0e-9,2.00e-9,0.05e-9)
epsilon = np.arange(0.0e-9, 1.0e-9, 0.05e-9)
#epsilon = np.arange(0.0e-9,1.1e-9, 0.05e-9)
#w0array = np.arange(0e-9,2.1e-9, 0.1e-9)
#warray = np.arange(1.5e-9, 2.51e-9,0.5e-10)
...
for tr in range(1,1001):
    TR = str(tr)
    with open('csfracal_{0}.pkl'.format(TR),'rb') as f: 
        csfrac[:,:,tr-1] = np.squeeze(pickle.load(f,encoding = 'latin1'))
    with open('isfracal_{0}.pkl'.format(TR),'rb') as f:
...
timea = np.arange(0,20000,0.1)
def expo(t,k,a0,ab):
    y = 12e-10*(0.1*np.exp(-k*t) + 0.3*np.exp(-k*t/20.0) )+ ab 
    return y
plt.rcParams.update({'font.size': 14})
expon = expo(timea, 0.0005, 0.01, 1.40e-9)
expon2 = expo(timea-4000,0.0005, 0.1, 1.40e-9)
plt.plot(timea/3600, expon, linewidth = 2,color = "forestgreen", label ="CS")
plt.plot(timea/3600, expon2,linewidth = 2, color = "gold", label = "IS" )
plt.xlabel("time since IS (hours)")
...
#pylab.ticklabel_format(axis='x',style='sci',scilimits=(-9,-8))
plt.imshow(alldatacs,origin='lower',extent = (min(epsilon), 1e-9, min(warray), max(warray)),aspect = 'auto')
plt.plot(expon2-expon, expon, color = "red",zorder = 2, linewidth  = 2)

clb = plt.colorbar()
clb.ax.set_title('time in CS state',fontsize=12)
plt.xlabel(r"$\Delta$")
plt.ylabel("recurrent strength (CS)") 
plt.show()


plt.imshow(alldatais,origin='lower',extent = (min(epsilon), max(epsilon), max(warray), min(warray)),aspect = 'auto')
clb = plt.colorbar()
clb.ax.set_title('time in IS state',fontsize=8)
plt.xlabel(r"$\Delta$")
plt.ylabel("recurrent strength") 
plt.show()
...
for tr in range(1,101):
    TR = str(tr)
    with open('csfracalo_{0}.pkl'.format(TR),'rb') as f: 
        csfrac[:,tr-1] = np.squeeze(pickle.load(f,encoding = 'latin1'))
    with open('isfracalo_{0}.pkl'.format(TR),'rb') as f:
        isfrac[:,tr-1] = np.squeeze(pickle.load(f,encoding = 'latin1'))
...
timea = np.arange(0,20000,0.1)
def expo(t,k,a0,ab):
    y = 25e-10*(0.3*np.exp(-k*t/0.5) + 0.1*np.exp(-k*t/40.0) )+ ab 
    return y
...
popt, pcov = curve_fit(expo2, 10**10*warray, np.mean(csfrac[:,:],1), method='dogbox')

plt.plot(warray,np.mean(csfrac[:,:],1),'o')
#plt.show()
plt.plot(warray, expo2(10**10*warray, *popt))
plt.show()

trajc = expo2(10**10*expon,*popt)
plt.plot(timea/3600,trajc, color = "k",linewidth = 2, label = "CS only" )


print(len(epsilon2),len(warray2))
Z_traj= rbf(expon2-expon, expon)
plt.plot(timea/3600, Z_traj,color = "red",linewidth = 2, label = "CS with IS")
plt.xlabel("time since IS (hours)")
