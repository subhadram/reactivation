...
def main(trial):
    ...
    aarray = np.arange(1.5e-9,1.51e-9,5e-11)
    epsilon = np.arange(0.0e-9, .01e-9, 10e-11)
        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
               ...
               """
                ...
                alpha = np.sqrt(0.55)
                r_factorg1 = alpha*r_factor1 + alpha*r_factor3
                r_factorg1 = np.maximum(r_factorg1 + 0.5, 0)
                
                r_factorg2 = alpha*r_factor2 + alpha*r_factor3
                ...
                noise_EE[0:group1ei,:] = noise_EE[0:group1ei,:] * (np.random.rand(Ng1, Nt) < np.outer(np.ones(Ng1), r_factorg1))
                noise_EE[group2si:group2ei,:] = noise_EE[group2si:group2ei,:] * (np.random.rand(Ng2, Nt) < np.outer(np.ones(Ng2), r_factorg2))
                noise_EE[group2ei:inhind,:] = noise_EE[group2ei:inhind,:] * (np.random.rand(int(0.05*N), Nt) < np.outer(np.ones(int(0.05*N)), r_factorg3)) #0.05 is the portion of excitatory neurons not in either ensemble.
                ...
                allrates[1,:] = rateg2
                allrates[2,:] = rateov
                pickle.dump(allrates,open('allratesuc055_{0}.pkl'.format(TR),'wb'))
...
#                 after1 = []
#                 before1= []
#                 triggered_average1 = np.zeros((len(event_indices), 10000))
#                 for ie,ei in enumerate(event_indices):
#                     during1.append(np.mean(rateov[ei:ei+500]))
#                     after1.append(np.mean(rateov[ei+500:ei+10000]))
#                     before1.append(np.mean(rateov[ei-500:ei]))
#                     if (ei > 10000) and all( xr <10 for xr in rateg2[ei-10000:ei]):
#                         triggered_average1[ie,: ] = rateov[ei-10000:ei]
#                         fig = plt.figure(ie)
#                         plt.plot(triggered_average1[ie,:])
#                         #plt.plot(tvector, rateg2)
#                         #plt.plot(tvector, rateov)
#                         plt.title(r" $\epsilon$ = "+ EP)
#                         plt.savefig('cstrigger2_%s_%s.png'%(EP,ie))
#                         #plt.show()
#                         plt.close(fig)
#                     else:
#                         triggered_average1[ie,: ] = np.nan
...
#                 after2 = []
#                 before2 = []
#                 triggered_average2 = np.zeros((len(event_indices2), 10000))
#                 for ie2,ei2 in enumerate(event_indices2):
...
#                     if (ei2 > 10000) and all( xr <10 for xr in rateg1[ei2-10000:ei2]):
#                         triggered_average2[ie2,: ] = rateov[ei2-10000:ei2]
#                         fig = plt.figure(ie2)
#                         plt.plot(triggered_average2[ie2,:])
#                         #plt.plot(tvector, rateg2)
#                         #plt.plot(tvector, rateov)
#                         plt.title(r" $\epsilon$ = "+ EP)
#                         plt.savefig('istrigger2_%s_%s.png'%(EP,ie2))
#                         #plt.show()
#                         plt.close(fig)
                        
#                     else:
#                         triggered_average2[ie2,: ] = np.nan
#                 staf = plt.figure(10000)
#                 tttime = np.arange(-int(10000*dt),0,dt)
#                 plt.plot(tttime,np.nanmean(triggered_average1,0), label = "CS")
#                 plt.plot(tttime,np.nanmean(triggered_average2,0), label = "IS")
#                 plt.ylabel("inhibitory firing rate ")
#                 plt.xlabel("time before event")
#                 plt.title(r" $\epsilon$ = "+ EP)
#                 plt.legend()
#                 plt.savefig('sta2_%s.png'%(EP))
#                 plt.close(staf)
                
#                 timescs = timevec[event_indices]
#                 timesis = timevec[event_indices2]
#                 print(timescs,timesis)
#                 difft = []
...
#     # pickle.dump(mratecsd,open('dratecs_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(mrateisd,open('drateis_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(mratecsb,open('bratecs_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(mrateisb,open('brateis_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(mratecsa,open('aratecs_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(mrateisa,open('arateis_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(csfrac,open('mcsfrac_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(isfrac,open('misfrac_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(lencs,open('numcs_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(lenis,open('numis_{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(csrate,open('mcsrate{0}.pkl'.format(TR),'wb'))
#     # pickle.dump(israte,open('misrate_{0}.pkl'.format(TR),'wb'))
