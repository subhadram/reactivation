...
warray = np.arange(1.5e-9, 2.6e-9,1e-10)
...
for tr in range(1,101):
    TR = str(tr)
    with open('csfraca2_{0}.pkl'.format(TR),'rb') as f: 
        csfrac[:,:,tr-1] = np.squeeze(pickle.load(f,encoding = 'latin1'))
    with open('isfraca2_{0}.pkl'.format(TR),'rb') as f:
...
plt.plot(warray, slopes/slopes1, label ="CS")
