...
warray = np.logspace(-3,3, num =101)
print(warray)
...
for tr in range(1,101):
    TR = str(tr)
    with open('csfracf3_{0}.pkl'.format(TR),'rb') as f: 
        csfrac[:,:,tr-1] = np.squeeze(pickle.load(f,encoding = 'latin1'))
    with open('isfracf3_{0}.pkl'.format(TR),'rb') as f:
...
#yaxi = np.linspace(min(slopes), max(slopes1))
#X, Y = np.meshgrid(warray, yaxi)
#plt.pcolormesh(X,Y,timespent)
#plt.imshow(timespent , extent=(min(warray), max(warray), min(np.abs(slopes)), max(np.abs(slopes1))), aspect = 'auto')
#plt.xscale('log')
plt.plot(warray,(slopes),color="forestgreen",linewidth=4, zorder = 2, label = "CS")
plt.plot(warray,(slopes1),color="gold",linewidth=4, zorder = 2, label = "IS")
#ax2.set_xticks([])
#ax2.set_yticks([])
