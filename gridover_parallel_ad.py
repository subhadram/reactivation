...
def main(trial):
    ...
    aarray = np.arange(1.0e-9,1.51e-9,.025e-9)
    epsilon = np.arange(0.0e-9, 0.21e-9, 0.1e-10)
    ...
    for k in range(len(aarray)):   
        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
                ...
                iind[group2ei:] = int(1)
                inhind = int(0.75*N)
                Jeid = np.random.normal(Jei, 0.1e-9, (int(N-inhind),inhind))
                Jeid = np.where(Jeid>0, Jeid, 0)
                Jied = np.random.normal(Jie, 0.1e-9, (inhind, int(N-inhind)))
                Jied = np.where(Jied >0, Jied, 0)
                Jee1d = np.random.normal(Jee1, 0.1e-9, (group1ei,group1ei))
                Jee1d = np.where(Jee1d >0, Jee1d, 0)
                Jee2d = np.random.normal(Jee2, 0.1e-9, (int(group2ei-group2si),int(group2ei -group2si)))
                Jee2d = np.where(Jee2d >0, Jee2d, 0)
                J[inhind:,:inhind] = Jeid
                J[:inhind,inhind:] = Jied
                J[:group1ei,:group1ei] = Jee1d
                J[group2si:group2ei,group2si:group2ei ] =  Jee2d
                dw = dw0*eind
                NE = inhind
                NI = N-NE
                Nt = len(tvector)
               ...
               """
                ...
                plt.title(r" $\epsilon$ = "+ EP + AA)
                plt.savefig('frate_%s_%s_%s.png'%(EP,TR,AA))
                plt.close(fig)
    ...
    pickle.dump(csfrac,open('csfracadg_{0}.pkl'.format(TR),'wb'))
    pickle.dump(isfrac,open('isfracadg_{0}.pkl'.format(TR),'wb'))
    pickle.dump(csrate,open('csrateadg_{0}.pkl'.format(TR),'wb'))
    pickle.dump(israte,open('israteadg_{0}.pkl'.format(TR),'wb'))
