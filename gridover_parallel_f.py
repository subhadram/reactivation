...
def main(trial):
    ...
    aa = np.arange(1.5e-9,1.6e-9,1e-10)
    epsilon = np.arange(-0.2e-9, 0.21e-9, 1e-10)
    overlaps = np.arange(0.0, 0.06, 0.05)
    farray = np.arange(4, 41, 4)
    firingrates = np.zeros((len(farray),len(overlaps),trials,3,len(tvector)))
    csfrac = np.zeros((len(farray),len(epsilon), trials))
    isfrac = np.zeros((len(farray),len(epsilon), trials))
        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
               ...
               """
                ...
                plt.savefig('ffrate_%s_%s.png'%(EP,TR))
                plt.close(fig)
    ...
    pickle.dump(csfrac,open('csfracf_{0}.pkl'.format(TR),'wb'))
    pickle.dump(isfrac,open('isfracf_{0}.pkl'.format(TR),'wb'))
