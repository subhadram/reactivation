...
def main(trial):
    ...
    aarray = np.arange(1.5e-9,1.51e-9,1e-10)
    epsilon = np.arange(0.3e-9, 0.31e-9, 1e-10)
        ...
        for eps in range(len(epsilon)):
            ...
            for j in range(trials):
               ...
               """
                ...
                #fig=plt.figure()
                #ax1 = plt.subplot(211)
                #ax1.plot(tvector, gnoise_EE[0,:],color = "k", linewidth = 0.5)
                ...
                #plt.xlim(0,4)
                plt.show()
                plt.plot(tvector, 10**3*V[0,:], color = "forestgreen",linewidth = 0.5)
                plt.plot(tvector, 10**3*V[200], color = "gold",linewidth = 0.5)
                ...
                fig = plt.figure()
                ax1 = plt.subplot(311)
                ax1.plot(tvector, 10**3*V[0,:], color = "forestgreen",linewidth = 0.5)
                ax1.plot(tvector, 10**3*V[200], color = "gold",linewidth = 0.5)
                #ax1.plot(tvector, V[170], color = "blue")
                plt.ylabel("Voltage (mV)")
                ax = plt.subplot(312,sharex = ax1)
                for ner in range(0,5):
                    spikeind = np.argwhere(spikes[ner,:]>0.0)
                    spikes[ner,spikeind] = spikes[ner,spikeind]+nerd
                    
                    ax.scatter(tvector,spikes[ner,:],color = 'forestgreen', marker = "|", label = "CS neurons")
                    nerd = nerd + 1
                for ner in range(170, 172):
                    spikeind = np.argwhere(spikes[ner,:]>0.0)
                    spikes[ner,spikeind] = spikes[ner,spikeind]+nerd
                    
                    ax.scatter(tvector,spikes[ner,:],color = 'blue', marker = "|", label = "overlapping neurons")
                    nerd = nerd + 1
                for ner in range(200, 205):
                    spikeind = np.argwhere(spikes[ner,:]>0.0)
                    spikes[ner,spikeind] = spikes[ner,spikeind]+nerd
                    
                    ax.scatter(tvector,spikes[ner,:],color = 'gold', marker = "|", label = "IS neurons")
                    nerd = nerd + 1
                
                handles, labels = ax.get_legend_handles_labels()
                display = (0,5,11)
                plt.ylim(0.5, nerd)
                plt.legend([handle for i,handle in enumerate(handles) if i in display],[label for i,label in enumerate(labels) if i in display],bbox_to_anchor=(0, 1.02, 1, 0.2), loc="lower left",mode="expand", borderaxespad=0, ncol=3)
                #plt.xlabel("time (s)")
                #plt.xlim(0,4)
                plt.ylabel("neuron")
                ax2 = plt.subplot(313, sharex = ax1)
                ax2.plot(tvector, rateg1, color = "forestgreen", label = "CS")
                ax2.plot(tvector, rateg2, color = "gold", label = "IS")
                plt.xlabel("Time (s)")
                plt.ylabel("Firing rate (Hz)")
                plt.show()
    ...
    pickle.dump(csfrac,open('csfraca2_{0}.pkl'.format(TR),'wb'))
    pickle.dump(isfrac,open('isfraca2_{0}.pkl'.format(TR),'wb'))
