...
aa = np.arange(2.2e-9,2.21e-9,1e-10)
...
csfrac = np.zeros((len(aa),len(epsilon), trials))
isfrac = np.zeros((len(aa),len(epsilon), trials))
csrate = np.zeros((len(aa),len(epsilon), trials))
israte = np.zeros((len(aa),len(epsilon), trials))
...
for k in range(len(aa)):   
    ...
    for eps in range(len(epsilon)):
        ...
        for j in range(trials):
            ...
            ds = 1
            group0si = 0
            group0ei = int((group0)*N)
            group1si = int((group0)*N)
            group1ei = int((group0+group1)*N)
            group2si = int((group0+group1-overlap)*N)
            group2ei = int((group0+group1+group2- overlap)*N)
            eind = np.zeros(N)
            ...
            J[inhind:,:inhind] = Jei
            J[group0si:group0ei,inhind:] = Jie
            J[group1si:inhind,inhind: ] = Jie2
            J[group0si:group0ei,group0si:group0ei ] =  Jdd
            J[group1si:group1ei,group1si:group1ei] = Jee1
            J[group2si:group2ei,group2si:group2ei ] =  Jee2
            J[group1si:group1ei,group0si:group0ei] = Jd1
            J[group2si:group2ei,group0si:group0ei] = Jd2
            J[group0si:group0ei,group1si:group1ei] = J1d
            J[group0si:group0ei,group2si:group2ei] = J2d
            J[group2si:group2ei,group1si:group1si] = J12
            J[group1si:group1si,group2si:group2si] = J21
            ...
            #plt.show()
            spikesg0 = np.mean(spikes[0:group0ei,:],0)
            ...
            rateg0 = spikes_to_rate(spikesg0, dt, 0.02)
            fig = plt.figure(1)
            plt.plot(tvector, rateg1, color = "forestgreen", label = "CS")
            plt.plot(tvector, rateg2,color = "gold", label ="IS")
            plt.plot(tvector, rateg0)
            plt.title(r" $\epsilon$ = "+ EP)
            plt.savefig('rate_%s.png'%(EP))
            plt.legend()
            plt.close(fig)
...
pickle.dump( csfrac, open( "csfrac3s.pkl","wb"))
pickle.dump( isfrac, open( "isfrac3s.pkl","wb"))
pickle.dump( csrate, open( "csrate3s.pkl","wb"))
pickle.dump( israte, open( "israte3s.pkl","wb"))
plt.errorbar(epsilon, np.mean(csfrac[0,:,:],1), yerr =np.std(csfrac[0,:,:],1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5,color = "forestgreen", label = "CS")
plt.errorbar(epsilon, np.mean(isfrac[0,:,:],1), yerr =np.std(isfrac[0,:,:],1)/np.sqrt(trials),solid_capstyle='projecting', capsize=5,color = "gold", label = "IS")
